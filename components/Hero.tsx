...
const copy = {
  th: {
    ...
    skills: ["Graphic Design", "Video Editing", "Programming", "Brand Systems"],
  },
  en: {
    nav: [
      ["About", "#about"],
    ...
    eyebrow: "Portfolio 2026",
    role: "Graphic Designer · Video Editor · Programmer",
    desc: "I help brands grow with beautiful design, high-converting videos, and practical web systems — all in one creative workflow.",
...
export default function Hero({ lang, onToggleLang }: { lang: Lang; onToggleLang: () => void }) {
  const t = copy[lang];
  ...
  return (
    <section className="relative min-h-screen overflow-hidden px-4 py-6 sm:px-6">
      ...
      <nav className="section-shell glass-panel sticky top-4 z-50 flex items-center justify-between rounded-3xl px-4 py-3 sm:px-5">
        <a href="#" className="flex items-center gap-3">
          ...
          <span className="hidden text-sm font-black text-slate-900 sm:block">Narongrit</span>
        </a>
        <div className="hidden items-center gap-1 md:flex">
          {t.nav.map(([label, href]) => (
            <a key={href} href={href} className="rounded-2xl px-4 py-2 text-sm font-bold text-slate-600 transition hover:bg-white/70 hover:text-blue-700">
              {label}
            </a>
          ))}
        </div>
  ...
  {t.hire}
      ...
      <div className="section-shell grid min-h-[calc(100vh-96px)] items-center gap-12 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:py-20">
        <div className="text-center lg:text-left">
          <div className="eyebrow reveal"><Icon name="sparkles" className="h-4 w-4" /> {t.eyebrow}</div>
          <h1 className="reveal delay-1 mt-6 font-display text-[clamp(3.15rem,11vw,7rem)] font-black leading-[0.9] tracking-[-0.08em]">
          ...
          <div className="reveal delay-4 mt-8 grid gap-3 sm:mx-auto sm:max-w-md sm:grid-cols-2 lg:mx-0">
            <a href="#portfolio" className="btn-primary px-7 py-3.5">{t.viewWorks}<Icon name="arrowDown" className="h-5 w-5" /></a>
            <a href="#showreel" className="btn-secondary px-7 py-3.5">{t.showreel}<Icon name="play" className="h-4 w-4" /></a>
          </div>

          <div className="mt-10 grid grid-cols-3 gap-3 sm:mx-auto sm:max-w-lg lg:mx-0">
            {t.stats.map(([num, label]) => (
              <div key={label} className="glass-panel rounded-3xl px-3 py-4 text-center">
                <div className="font-display text-2xl font-black text-blue-800 sm:text-3xl">{num}</div>
                <div className="mt-1 text-[0.68rem] font-bold text-slate-500 sm:text-xs">{label}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="glass-panel rounded-[1.7rem] p-5 sm:col-span-2">
                <div className="flex items-center gap-4">
                  <div className="grid h-16 w-16 place-items-center rounded-3xl bg-gradient-to-br from-sky-200 to-blue-600 text-white shadow-blue-glow float-y"><Icon name="sparkles" className="h-8 w-8" /></div>
                  <div>
                    <div className="font-display text-xl font-black text-white sm:text-2xl">{t.mockTitle}</div>
                    <div className="mt-1 text-sm font-semibold text-sky-100/80">{t.mockDesc}</div>
                  </div>
                </div>
              </div>

              {t.skills.map((skill, index) => (
                <div key={skill} className="rounded-[1.4rem] border border-white/15 bg-white/10 p-4 backdrop-blur">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="text-sm font-extrabold text-white">{skill}</span>
                    <span className="text-xs font-black text-sky-200">0{index + 1}</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-white/10">
                    <div className="h-full rounded-full bg-gradient-to-r from-sky-300 to-white" style={{ width: `${92 - index * 5}%` }} />
                  </div>
                </div>
              ))}
            </div>
